...
public class AdminLoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // 设置响应的Content-Type为text/plain，确保返回的是纯文本
        ...
        if (username != null && password != null) {
            ...
            if (username.equals(adminUsername) && password.equals(adminPassword)) {
                ...
                response.setStatus(HttpServletResponse.SC_OK);  // 设置响应状态码为200（成功）
                response.getWriter().write("Login Successful!");  // 返回成功消息
            } else {
                // 登录失败，返回失败消息
                System.out.println("Invalid username or password.");

                response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);  // 设置响应状态码为401（未授权）
                response.getWriter().write("Invalid username or password.");  // 返回失败消息
            }
        } else {
            // 如果用户名或密码为空，返回错误消息
            System.out.println("Username or password is empty!");

            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);  // 设置响应状态码为400（坏请求）
            response.getWriter().write("Username or password cannot be empty.");  // 返回错误消息
        }
    }
